...
for root, dirs, files in os.walk('data/scape/shapes_test/'):
    for file in files:
        for i in range(num):
            ...
            target_normal = compute_vertex_normals(verts,faces.to(device))

            N = verts.shape[0]
            mu = 0.01  # 高斯分布的均值
            sigma = 0.01 + i*0.01 # 高斯分布的标准差
            noise = [[random.gauss(mu, sigma) for _ in range(3)] for _ in range(N)]
            noise = torch.tensor(noise).float().to(device)
            noise_vertices = verts +  noise.mul(target_normal)
            # rot  = get_sampled_rotation_matrices_by_axisAngle(3)
            # rotated_vertices = torch.matmul(verts, rot[i])
            save_path_mesh = os.path.join(save_path ,save_name, 'noi'+str(i+1))
            if not os.path.exists(save_path_mesh):
                os.makedirs(save_path_mesh)
            save_mesh = o3d.geometry.TriangleMesh()
            save_mesh.vertices =  o3d.utility.Vector3dVector(noise_vertices.squeeze().detach().cpu().numpy())
