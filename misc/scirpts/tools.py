...
def get_sampled_rotation_matrices_by_axisAngle(batch,angle):
    
    theta = torch.autograd.Variable(torch.FloatTensor(np.random.uniform(-angle/2,angle/2, batch)*np.pi).cuda()) #[0, pi] #[-180, 180]
    sin = torch.sin(theta)
