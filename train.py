...
import os
import torch
from models.dataset import Dataset, shape_to_device
from models.loss import GraphDeformLoss_Neural
...
def train_net(cfg):
    ...
    if cfg["dataset"]["type"] == "vts":
        train_dataset = Dataset(dataset_path_train, name=cfg["dataset"]["name"],use_cache=True, train=True,with_dino=cfg["with_dino"],feat_mat=cfg["feat_mat"])

        test_dataset = Dataset(dataset_path_test, name=cfg["dataset"]["name"], use_cache=True,train=False,with_dino=cfg["with_dino"],feat_mat=cfg["feat_mat"])
